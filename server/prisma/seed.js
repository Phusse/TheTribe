...
async function main() {
    ...
    const m1 = await database_1.prisma.trainingModule.upsert({
    ...
    const m2 = await database_1.prisma.trainingModule.upsert({
    ...
    // Seed live sessions
    await database_1.prisma.liveSession.createMany({
        data: [
            {
                id: "session-001",
    ...
    for (const g of groups) {
        await database_1.prisma.group.upsert({
            where: { id: g.id },
            update: {},
            create: g,
        });
    ...
    const adminPassword = await (0, password_1.hashPassword)("Admin@123456");
    ...
    // Seed a test member
    const memberPassword = await (0, password_1.hashPassword)("Member@123456");
    const member = await database_1.prisma.user.upsert({
