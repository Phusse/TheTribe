...
export const getHistoryController = async (
  ...
  try {
    ...
    const history = await messagesService.getMessageHistory(req.user!.sub, partnerId);
...
export const sendMessageController = async (
  ...
  try {
    const { partnerId } = req.params;
    const { text } = req.body;
    const message = await messagesService.sendMessage(req.user!.sub, partnerId, text);
